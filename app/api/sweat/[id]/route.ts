...
export async function PATCH(
  ...
  try {
    ...
    const { data, error } = await db
      .from('sweat_tasks').update(patch).eq('id', id).select().single()
    ...
    return serverError('sweat.PATCH', err)
  }
}
...
export async function DELETE(
  ...
  try {
    const db = createAdminClient()
    const { error } = await db.from('sweat_tasks').delete().eq('id', id)
    if (error) throw error
    return NextResponse.json({ ok: true })
  } catch (err) {
    return serverError('sweat.DELETE', err)
  }
}
